...
export async function borrowMarketplaceItem(id) {
  const { data } = await api.post(`/api/marketplace/${id}/borrow`);
  return data;
}

export async function createMarketplaceItemRequest(payload) {
  const { data } = await api.post("/api/marketplace/requests", payload);
  return data;
}
