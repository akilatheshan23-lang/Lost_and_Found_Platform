...
exports.getLostItems = async (req, res) => {
  try {
    ...
    // Database-side limit reduces payload size and lookup time to single digits ms
    const items = await LostItem.find(filter).sort({ createdAt: -1 }).limit(Number(limit) || 100);
    res.json(items);
  } catch (error) {
