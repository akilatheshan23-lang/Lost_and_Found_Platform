    ...
    <div className="card-solid card-hover overflow-hidden">
      <div className="p-5 space-y-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <span className={`text-xs px-3 py-1 rounded-full font-medium border ${categoryBadge(item.category)}`}>
            {item.category}
          </span>
        </div>
        ...
        {imgSrc ? (
          <div className="rounded-2xl overflow-hidden border border-slate-200">
            <img src={imgSrc} alt={item.title} className="w-full max-h-[420px] object-cover" loading="lazy" />
          </div>
        ) : null}
